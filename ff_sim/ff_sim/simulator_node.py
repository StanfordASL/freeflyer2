...
class FreeFlyerSimulator(Node):
    ...
    def sim_loop(self) -> None:
        ...
        mocap.pose.position.x = self.x_cur[0] + np.random.normal(loc=0., scale=noise_xy)
        mocap.pose.position.y = self.x_cur[1] + np.random.normal(loc=0., scale=noise_xy)
        theta = self.x_cur[2] + np.random.normal(loc=0., scale=noise_theta)

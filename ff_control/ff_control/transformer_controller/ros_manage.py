...
from ff_control.transformer_controller.adapter import FrozenTextAdapter
from ff_control.transformer_controller.art import AutonomousFreeflyerTransformer_Lang_ctg, AutonomousFreeflyerTransformer_Lang
...
'''
def get_DT_model(model_name, n_state, n_action, ctg_condition):
    # DT model creation
    ...
    accelerator.load_state(str(root_folder) + '/saved_files/checkpoints/' + model_name)

    # load text encode also
    # load the text encoders for each models (and its weight)  
    MODEL = "distilbert-base-uncased"
    text_encoder_ctg = FrozenTextAdapter(model_name=MODEL, out_dim=config.hidden_size, output_mode="tokens").to(device).eval()
    text_encoder_ctg.load_adapter(root_folder + '/saved_files/checkpoints/' + model_name + '/text_adapter.pth') 

    return model.eval(), text_encoder_ctg
...
def state_init_final2sample(data_stats, state_init, state_final, final_time=T_nominal, obs=obs_nominal, n_obs=n_obs_nominal):
    ...
    behavior_i = torch.tensor([0])[None,:]
    command_id_i = torch.tensor([0])[None,:]
    
    return (states_i, actions_i, rtgs_i, ctgs_i, goal_i, timesteps_i, attention_mask_i, dt_i, time_sec_i, ix, behavior_i, command_id_i)
...
def ros_model_inference_dyn(model, data_stats, state_init, state_final, text_encoder, rtg_perc=1., ctg_condition=True, ctg_perc=1., rtg=None, ctg_clipped=True, text_command = None):
    ...
    if ctg_condition:
        states_i, actions_i, rtgs_i, ctgs_i, goal_i, timesteps_i, attention_mask_i, dt, time_sec, ix, _, _ = state_init_final2sample(data_stats=data_stats,state_init=state_init, state_final=state_final)

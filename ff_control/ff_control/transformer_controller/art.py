...
class AutonomousFreeflyerTransformer_VarObs(DecisionTransformerPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embed_constraint = torch.nn.Linear(1, config.hidden_size)
        self.embed_state = torch.nn.Linear(config.state_dim, config.hidden_size)
        if config.embed_entire_observation:
            self.embed_observation = nn.Linear(config.obs_dim, config.hidden_size)
        else:
            self.embed_observation = nn.Linear(config.single_obs_dim, config.hidden_size)
        self.embed_action = torch.nn.Linear(config.act_dim, config.hidden_size)
    ...
    def forward(
        ...
        # embed each modality with a different head
        state_embeddings = self.embed_state(states)
        if self.config.embed_entire_observation:
        ...
        if self.config.embed_entire_observation:
            ...
            observation_embeddings = torch.stack(observation_embeddings).sum(axis=0) + time_embeddings
        action_embeddings = action_embeddings + time_embeddings
        goal_embeddings = goal_embeddings + time_embeddings
        returns_embeddings = returns_embeddings + time_embeddings
        constraints_embeddings = constraints_embeddings + time_embeddings
        ...
        stacked_inputs = (
            torch.stack((goal_embeddings, returns_embeddings, constraints_embeddings, state_embeddings,
                         observation_embeddings, action_embeddings), dim=1)
            .permute(0, 2, 1, 3)
            .reshape(batch_size, 6 * seq_length, self.hidden_size)
        )
        stacked_inputs = self.embed_ln(stacked_inputs)
        ...
        # to make the attention mask fit the stacked inputs, have to stack it as well
        stacked_attention_mask = (
            torch.stack((attention_mask, attention_mask, attention_mask, attention_mask, attention_mask,
        ...
        # reshape x so that the second dimension corresponds to the original
        # goal (0), return_to_go (1), constraint_to_go (2), states (3), observations (4), or actions (5);
        # i.e. x[:,3,t] is the token for s_t
        x = x.reshape(batch_size, seq_length, 6, self.hidden_size).permute(0, 2, 1, 3)

        # get predictions
        state_preds = self.predict_state(x[:, 5])  # predict next state given (T+R+C)+state+observation and action
        action_preds = self.predict_action(x[:, 4])  # predict next action given (T+R+C)+state+observation
...
class AutonomousFreeflyerTransformer_VarObs_ConcatObservations(DecisionTransformerPreTrainedModel):
    """
    ...
    def forward(
        ...
        # time embeddings are treated similar to positional embeddings
        state_embeddings = state_embeddings + time_embeddings
        observation_embeddings_0 = observation_embeddings[0] + time_embeddings
        observation_embeddings_1 = observation_embeddings[1] + time_embeddings
        observation_embeddings_2 = observation_embeddings[2] + time_embeddings
        observation_embeddings_3 = observation_embeddings[3] + time_embeddings
        action_embeddings = action_embeddings + time_embeddings
        goal_embeddings = goal_embeddings + time_embeddings
        returns_embeddings = returns_embeddings + time_embeddings
        constraints_embeddings = constraints_embeddings + time_embeddings
        ...
        stacked_inputs = (
            torch.stack((goal_embeddings, returns_embeddings, constraints_embeddings, state_embeddings,
                         observation_embeddings_0, observation_embeddings_1, observation_embeddings_2, observation_embeddings_3, action_embeddings), dim=1)
            .permute(0, 2, 1, 3)
            .reshape(batch_size, 9 * seq_length, self.hidden_size)
        )
        stacked_inputs = self.embed_ln(stacked_inputs)

        # to make the attention mask fit the stacked inputs, have to stack it as well
        '''stacked_attention_mask = (
            torch.stack((attention_mask, attention_mask, attention_mask, attention_mask, 
                         attention_mask * (num_obstacles[0].view(-1, 1) > 0).float(),
                         attention_mask * (num_obstacles[0].view(-1, 1) > 1).float(),
                         attention_mask * (num_obstacles[0].view(-1, 1) > 2).float(),
                         attention_mask * (num_obstacles[0].view(-1, 1) > 3).float(),
                         attention_mask), dim=1)
            .permute(0, 2, 1)
            .reshape(batch_size, 9 * seq_length)
        )'''
        stacked_attention_mask = (
            torch.stack((attention_mask, attention_mask, attention_mask, attention_mask, 
                         attention_mask * (num_obstacles > 0).float(),
                         attention_mask * (num_obstacles > 1).float(),
                         attention_mask * (num_obstacles > 2).float(),
                         attention_mask * (num_obstacles > 3).float(),
                         attention_mask), dim=1)
            .permute(0, 2, 1)
            .reshape(batch_size, 9 * seq_length)
        )
        ...
        # goal (0), return_to_go (1), constraint_to_go (2), states (3), observations (4), or actions (5);
        # i.e. x[:,3,t] is the token for s_t
        x = x.reshape(batch_size, seq_length, 9, self.hidden_size).permute(0, 2, 1, 3)

        # get predictions
        state_preds = self.predict_state(x[:, 8])  # predict next state given (T+R+C)+state+observation and action
        action_preds = self.predict_action(x[:, 7])  # predict next action given (T+R+C)+state+observation
...
class AutonomousFreeflyerTransformer_Lang(DecisionTransformerPreTrainedModel):
    """
    ...
    def forward(
        ...
        # interleave (g_1, s_1, a_1, g_2, s_2, a_2, ...)  -> [B, 3*T, H]
        traj_embeddings = (
        ...
        # Now reshape 3T back to (3, T) with stream order: (goal, state, action)

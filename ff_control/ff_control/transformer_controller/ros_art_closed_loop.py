...
class ROSAutonomousFreeflyerTransformerMPC():
    '''
    ...
    ########## CONSTRUCTOR ##########
    def __init__(self, model, data_stats, n_steps, transformer_mode='dyn', ctg_clipped=True, scp_mode='hard'):
        ...
        self.data_stats = copy.deepcopy(data_stats)
        ...
        self.max_time_len = self.data_stats['actions_mean'].shape[0]

        # Initialize the history of all predictions
        self.__initialize_context_history()
    
    ########## CLOSED-LOOP METHODS ##########
    def warmstart(self, current_obs, timestep, rtg0=None, rtgs_i=None, ctg0=None):
        '''
        ...
        obs_positions = torch.tensor(ff.obs['position']).to(TTO_manager.device)
        obs_radii = torch.tensor((ff.obs['radius']+ff.robot_radius)*ff.safety_margin).to(TTO_manager.device)
        ...
        past_context = {
            ...
            'ctgs_context' : self.ctgs_context,
            'goal_context' : self.goal_context,
            'norm_goal_context' : self.norm_goal_context,
        }
        if self.transformer_mode == 'dyn':
            ART_trajectory, ART_runtime = self.__torch_model_inference_dyn_closed_loop(self.model, self.data_stats, time_context, past_context, obs_positions, obs_radii, ctg_clipped=self.ctg_clipped)
        else:
            ART_trajectory, ART_runtime = self.__torch_model_inference_ol_closed_loop(self.model, self.data_stats, time_context, past_context, obs_positions, obs_radii, ctg_clipped=self.ctg_clipped)
    ...
    def __initialize_context_history(self):
        ...
        self.norm_state_context = torch.zeros((1, self.max_time_len, self.N_STATE)).float().to(TTO_manager.device)
        self.state_context = torch.zeros((self.N_STATE, self.max_time_len)).float().to(TTO_manager.device)
        self.norm_action_context = torch.zeros((1, self.max_time_len, self.N_ACTION)).float().to(TTO_manager.device)
        self.dv_context = torch.zeros((self.N_ACTION, self.max_time_len)).float().to(TTO_manager.device)
        self.norm_goal_context = torch.zeros((1, self.max_time_len, self.N_STATE)).float().to(TTO_manager.device)
        self.goal_context = torch.zeros((self.N_STATE, self.max_time_len)).float().to(TTO_manager.device)
        self.rtgs_context = torch.zeros((1, self.max_time_len, 1)).float().to(TTO_manager.device)
        self.ctgs_context = torch.zeros((1, self.max_time_len, 1)).float().to(TTO_manager.device)
    ...
    ########## STATIC METHODS ##########
    @staticmethod
    def __torch_model_inference_dyn_closed_loop(model, data_stats, time_context, past_context, obs_positions, obs_radii, rtgs_i=None, ctg_clipped=True):

        # Extract the time information over the horizon
        dt = time_context['dt']
        t_i = time_context['t_i']
        n_steps = time_context['n_steps']
        t_f = min(t_i + n_steps, data_stats['actions_mean'].shape[0])
    ...
    @staticmethod
    def __torch_model_inference_ol_closed_loop(model, data_stats, time_context, past_context, obs_positions, obs_radii, rtgs_i=None, ctg_clipped=True):
    ...
    @staticmethod
    def __ocp_scp_closed_loop(state_ref, action_ref, state_init, state_final, state_end_ref, t_i, t_f, ffm:FreeflyerModel, trust_region, scp_mode):
